...
template<typename T>
void Matrix<T>::tuck_rows(const std::vector<std::vector<T>> &v) {
    this->tuck_rows(Matrix<T> {v});
}

template<typename T>
void Matrix<T>::tuck_cols(const std::vector<std::vector<T>> &v) {
    this->tuck_cols(Matrix<T> {v});
}
